...
# 이메일 인증 요청 API
@member_bp.route('/api/request-verification', methods=['POST'])
...
# 이메일 인증 확인 API
@member_bp.route('/api/verify', methods=['POST'])
def verify():
    email = request.json.get('email')
...
def signup_page():
    verified_email = session.get('verified_email')
    if not verified_email:
        return redirect(url_for('member.main'))  # 이메일 인증 페이지로 리다이렉트
    return render_template('member/member_signup.html', email=verified_email)

@member_bp.route('/api/signup', methods=['POST'])
def signup_api():
    data = request.json
    required_fields = ['email', 'username', 'user_id', 'password', 'password_confirm', 'postal_code', 'address', 'add_detail', 'phone_number']
...
@member_bp.route('/api/check-id', methods=['POST'])
def check_id():
...
def login():
    ...
    try:
        connection = get_db_connection()
        with connection.cursor() as cursor:
            cursor.execute("SELECT id, user_id, password, isadmin FROM users WHERE user_id = %s", (user_id,))
...
# step 1
@member_bp.route('/api/request-reset-code', methods=['POST'])
...
# step 2
@member_bp.route('/api/verify-reset-code', methods=['POST'])
...
#step3
@member_bp.route('/api/reset-password', methods=['POST'])
def reset_password():
    data = request.json
